...
- `BaseMapLayer` は親 `group` の回転で XY → XZ を変換
...
`BaseMapLayer` は fetch、GeoJSON 展開、サンプリング、geometry 生成、描画をまとめて持っている。`MovingEntitiesLayer` も mock データ生成、compute 初期化、描画を一体化している。
